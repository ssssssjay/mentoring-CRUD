...
const checkAuthorization = async (req, res, next) => {
  try {
    ...
    if (!verifiedAccess) throw new Error("인증에 실패했습니다. 다시 로그인해주세요");
    ...
    if (verifiedAccess === "expiredToken") {
      ...
      const newVerifiedAccess = verifyToken(newAccessToken);
      req.verifiedToken = newVerifiedAccess;
    } else {
      console.log("액세스토큰이 만료되지 않아 액세스 토큰만을 확인합니다");
      req.verifiedToken = verifiedAccess;
    }
